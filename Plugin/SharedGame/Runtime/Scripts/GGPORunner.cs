...
namespace SharedGame {
    ...
    public class GGPORunner : IGameRunner {
        ...
        public void Init(IList<Connections> connections, int playerIndex, bool isSyncTest) {
            var remote_index = -1;
            ...
            //else
            {
                ...
                Init(connections[playerIndex].port, num_players, players, num_spectators, isSyncTest);
            }
        }
        ...
        public void Init(int localport, int num_players, IList<GGPOPlayer> players, int num_spectators, bool isSyncTest) {
            ...
            if (isSyncTest)
			{
				result = GGPO.Session.TestStartSession(
						OnBeginGameCallback,
			...
			{
				result = GGPO.Session.StartSession(
						OnBeginGameCallback,
						OnAdvanceFrameCallback,
						OnLoadGameStateCallback,
						OnLogGameState,
			...
			for (int i = 0; i < players.Count; i++) {
                var brHandle = new HouraiTeahouse.Backroll.BackrollPlayerHandle();

				CheckAndReport(GGPO.Session.AddPlayer(players[i], ref brHandle));

                if (players[i].type == GGPOPlayerType.GGPO_PLAYERTYPE_LOCAL) {
                    var playerInfo = new PlayerConnectionInfo();
                    playerInfo.handle = brHandle.Id;
                    ...
                    GameInfo.SetConnectState(brHandle.Id, PlayerConnectState.Connecting);
                    CheckAndReport(GGPO.Session.SetFrameDelay(brHandle.Id, FRAME_DELAY));
                }
